...
class ItemkuGetOrder extends Component
{
    ...
    public function render()
    {
        ...
        $orderss = ItemkuOrders::where('user_id', Auth::user()->id)->whereNotNull('profit')->get();
        $income = 0;
        foreach ($orderss as $val) {
            $income = $income + $val->profit;
        }
        $this->income = rupiah($income);
        ...
        if (isset($resp['data']['total_item'])) {
            ...
            $this->order = $resp['data']['total_item'];
        } else {
